...
export default function AddBusinessScreen() {
  ...
  const handleSave = async () => {
    // Basic validation
    if (!formData.businessName || !formData.brn || !formData.tin) {
      Alert.alert('Missing Fields', 'Please fill in all required business identity fields.');
      return;
    }
    ...
    try {
      const newBusiness: Business = {
        ...
        isPrimary: false, // Default to false, service will handle if it's the first one
        ...formData
      };

      await StorageService.addBusiness(newBusiness);
      router.back();
    } catch (e) {
